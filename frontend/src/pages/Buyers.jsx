...
export default function Buyers({ search, isAddModalOpen, setIsAddModalOpen, scope = 'all' }) {
  ...
  const fetchBuyers = async (isInitial = false) => {
    if (isInitial || !buyers.length) {
      setLoading(true);
    }
    try {
  ...
  const exportBuyersPDF = () => {
    ...
    const totalCount = displayBuyers.length;
        ...
        <head>
          ...
          <style>
            ...
            * { box-sizing: border-box; }
            ...
            .bank-badge { background: #e0f2fe; color: #0369a1; border: 1px solid #7dd3fc; font-weight: 800; }
            .cash-badge { background: #dcfce7; color: #15803d; border: 1px solid #86efac; font-weight: 800; }
        ...
        <body>
          <div class="no-print no-print-bar">
            <div style="display: flex; align-items: center; gap: 6px;">
              ...
              <span style="color: #94a3b8;">• Layout Controls (${totalCount} Buyers)</span>
            </div>
            <div style="display: flex; align-items: center; gap: 6px;">
          ...
          <script>
            const buyersData = ${JSON.stringify(displayBuyers)};
            const todayStr = "${todayStr}";
            const totalValuation = ${totalValuation};
            ...
            function applyPageSize(size) {
              ...
              const pageChunks = [];
              for (let i = 0; i < buyersData.length; i += size) {
                pageChunks.push(buyersData.slice(i, i + size));
              }
              ...
              container.innerHTML = pageChunks.map((chunk, pageIdx) => {
                ...
                return \`
                  <div class="sheet">
                    <div>
                      <div class="header">
                        ...
                        <div class="stats-inline">
                          <div class="stat-item">Total Buyers: <strong>\${buyersData.length} Leads</strong></div>
                      ...
                      <table>
                        ...
                        <tbody>
                          ...
                          \` : chunk.map((b, idx) => {
                            ...
                            const caseNo = b.isBankCase && b.id ? b.id.substring(0, 6).toUpperCase() : null;
                            return \`
                            <tr>
                    ...
                    <div class="footer">
                      AL ASR MOTORS Customer Care & Sales • Sheet \${pageIdx + 1} of \${totalPages} • Showing records \${chunk.length > 0 ? startIdx + 1 : 0} to \${startIdx + chunk.length} of \${buyersData.length}
                    </div>
                  </div>
                \`;
              }).join('');
          ...
          <div className="p-4 border-t border-white/10 flex flex-wrap items-center justify-between gap-4 font-mono text-xs text-slate-400 bg-slate-900/40">
            <div className="flex items-center space-x-3">
              <div>
                ...
                <strong className="text-cyan-400">{Math.min(currentPage * pageSize, displayBuyers.length)}</strong> of{' '}
